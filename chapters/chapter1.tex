...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{1.6cm}p{1.7cm}p{1.6cm}p{1.7cm}p{1.6cm}p{2.0cm}p{2.0cm}p{1.9cm}}
        ...
        \midrule
        水声通信 & 声波（kHz 量级） & 中远距离至超远距离 & 带宽较窄，速率较低 & 传播时延大 & 覆盖范围大、技术成熟度较高 & 速率受限，实时交互能力弱 & 远距离控制、低速监测、稀疏组网 \\
...
就本文关注的代表性场景而言，UWOC 的关键挑战主要可归纳为两类。第一类是低动态浑浊多径场景。在此类环境中，尽管收发平台相对运动较弱，但悬浮颗粒引起的吸收与散射不仅会造成光功率衰减，还会形成多重散射传播和非视距分量，从而带来明显的时延扩展和频率选择性衰落。第二类是高动态移动场景。在平台高速机动、姿态持续变化或相对速度波动明显的条件下，链路除了面临多径传播外，还会受到多普勒频移、动态指向偏差以及相干接收架构中相位噪声等因素的共同影响，导致系统传输与检测难度显著增加\cite{zengSurveyUnderwaterOptical2017, hamdullahModelingOptimizationNLOS2026, jaruwatanadilokUnderwaterWirelessOptical2008, zhuDopplerResistantOrthogonalChirp2022}。这种场景分化并不是为了方便叙述而人为划分，而是直接对应了物理层设计中“以多径为主”与“以多普勒和相位扰动为主”两条不同的问题链。图~\ref{fig:uwoc_scenario_challenges} 从典型应用链路与主要失真来源两个层面，对本文所关注的吸收散射、多径扩展与多普勒频移等关键挑战进行了直观示意。
...
综合来看，现有研究已分别在水下光信道建模、IM-DD 光 OFDM、AFDM 波形设计以及高动态相干传输等方面取得一定进展，但仍缺乏一条围绕 UWOC 典型场景特征展开的统一研究主线。表~\ref{tab:waveform_evolution_comparison} 对典型水下多载波波形从传统 OFDM 系列到 AFDM 及其索引调制扩展的关键特征进行了归纳。可以看到，技术演进的核心并不是单纯引入更多结构，而是围绕非负约束、多径鲁棒性、多普勒适应能力以及频谱/功率效率之间的平衡不断调整设计重心。特别是，如何将 AFDM 与光学接收架构和索引结构进一步结合，仍有待结合具体场景进行深化研究。这正是本文工作的主要切入点。
...
\begin{table}[htbp]
    ...
    \setlength{\tabcolsep}{2.6pt}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{p{1.6cm}p{1.3cm}p{1.7cm}p{1.5cm}p{1.1cm}p{1.1cm}p{1.5cm}p{1.7cm}}
        \toprule
        波形/方案 & 接收架构 & 核心结构 & 主要场景 & 抗多径 & 抗多普勒 & 效率特点 & 主要局限 \\
        \midrule
        OFDM & 直接/相干 & 傅里叶正交子载波 & 低动态多径 & 中等 & 较弱 & 频谱效率较高 & 高动态下ICI明显 \\
        ACO-OFDM & IM-DD & 面向非负约束的单极性扩展 & 低动态光链路 & 中等 & 较弱 & 功率效率较高 & 仍依赖傅里叶域结构 \\
        AFDM & 复数域接收 & 基于DAFT的啁啾映射 & 时延--多普勒耦合信道 & 较强 & 较强 & 结构可调、分集利用灵活 & 参数与检测更复杂 \\
        AFDM-IM & 相干/复数域 & AFDM与稀疏激活联合设计 & 高动态可靠传输 & 较强 & 较强 & 降低平均激活能量 & 检测与映射耦合较强 \\
        相干AFDM & 相干接收 & AFDM、索引调制& 高动态UWOC & 较强 & 强 & 保持有效数据维度 & 需处理噪声与频偏 \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
除上述两项核心工作外，本文还从统一理论基础出发，对水下光传播机理、双选择性信道模型、传统 OFDM 的局限性以及 AFDM 的基本原理进行了系统梳理，使第三章与第四章的场景化方案设计能够建立在一致的分析框架之上。综合而言，本文的研究内容覆盖了从低动态到高动态、从 IM-DD 到相干接收、从波形设计到检测算法的多个层面，但其内在逻辑始终围绕同一主线展开，即：针对 UWOC 不同代表性传播条件下的主导失真机制，对 AFDM 进行有针对性的系统适配与性能验证。图~\ref{fig:thesis_technical_roadmap} 进一步从总体逻辑上概括了本文由研究背景、关键难点、方案设计到性能验证的技术路线，以及第三章与第四章两条研究分支之间的递进关系。
