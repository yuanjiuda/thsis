...
\end{equation}
式(\ref{eq:silent_energy_direct}) 的物理含义明确：在候选频偏参数空间中搜索使静默位置残余能量最小的补偿值，即为对主导多普勒频偏的最优估计。当补偿值 $\theta$ 接近真实频偏时，被频偏推散到静默位置的能量将被最大程度地压缩回激活位置。
...
由图~\ref{fig:cost_func_convergence} 可以清晰观察到，代价函数呈现出典型的深 V 型谷底结构：当试探参数 $\theta$ 远离真实频偏 $\theta_0$ 时，未被正确补偿的线性相位斜坡导致大量能量从激活位置泄露至静默位置，代价函数值维持在较高水平；随着 $\theta$ 逐渐逼近 $\theta_0$，泄露能量被持续压缩，代价函数单调下降；当 $\theta=\theta_0$ 时，线性相位斜坡被完全消除，静默位置上的残余能量降至最低，代价函数取得尖锐的全局极小值。谷底的残余非零能量主要来自维纳相位噪声高阶随机残差 $\epsilon[q]$ 引起的不可消除扰动与热噪声贡献。
