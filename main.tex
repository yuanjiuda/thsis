...
verticalTitle = {面向复杂水下环境的ＡＦＤＭ无线光通信关键技术研究},    % 下面的例子展示了如何在竖排标题中使用垂直或者旋转的英文。
    ...
    industrialMentor = {叶方全高工},  % 行业合作导师
    industrialMentorEn = {PSE.Fangquan Ye},
    degree = 电子信息硕士专业学位,  % 申请类别
    degreeEn = {Master of Electronic Information},
    major = {新一代电子信息技术（含量子技术等）},  % 学位领域
    majorEn = {New-Generation Electronic \\Information Technology},
    % -------------
    %
...
% 让 PDF 默认展开书签面板，便于在阅读器中直接看到论文目录结构
\hypersetup{
...
\nocite{*} % 使未在正文中显式引用的条目排在已引用文献之后统一输出
\input{./misc/2_reference.tex} % 参考文献
% \input{./misc/3_appendices.tex} % 附录（当前版本不保留）
\input{./misc/4_pub.tex} % 攻读学位期间发表论文与研究成果清单
